...
import (
	...
	systemRes "github.com/MuserQuantity/gin-project-example/model/system/response"
...
func (b *BaseApi) ChangePassword(c *gin.Context) {
	...
	if err, _ := userService.ChangePassword(u, user.NewPassword); err != nil {
		global.SYS_LOG.Error("修改失败!", zap.Error(err))
...
func (b *BaseApi) SetUserInfo(c *gin.Context) {
	...
	if err, ReqUser := userService.SetUserInfo(user); err != nil {
		global.SYS_LOG.Error("设置失败!", zap.Error(err))
...
func (b *BaseApi) ResetPassword(c *gin.Context) {
	...
	if err := userService.ResetPassword(user.ID); err != nil {
		global.SYS_LOG.Error("重置失败!", zap.Error(err))
