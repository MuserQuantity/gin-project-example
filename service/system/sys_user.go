...
func (userService *UserService) Login(u *system.SysUser) (err error, userInter *system.SysUser) {
	...
	var user system.SysUser
	u.Password = utils.MD5V([]byte(u.Password))
	err = global.SYS_DB.Where("username = ? AND password = ?", u.Username, u.Password).Preload("Authorities").Preload("Authority").First(&user).Error
	return err, &user
}
...
func (userService *UserService) ResetPassword(ID uint) (err error) {
	err = global.SYS_DB.Model(&system.SysUser{}).Where("id = ?", ID).Update("password", utils.MD5V([]byte("123456"))).Error
	return err
}
