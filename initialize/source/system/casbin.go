...
func (c *casbin) CheckDataExist() bool {
	if errors.Is(global.SYS_DB.Where(adapter.CasbinRule{Ptype: "p", V0: "9528", V1: "GET", V2: "/user/getUserInfo"}).First(&adapter.CasbinRule{}).Error, gorm.ErrRecordNotFound) { // 判断是否存在数据
		return false
	}
	return true
}
