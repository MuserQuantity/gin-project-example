...
func (m *menu) Initialize() error {
	...
	if err := global.SYS_DB.Create(&entities).Error; err != nil { // 创建 model.User 初始化数据
...
func (m *menu) CheckDataExist() bool {
	if errors.Is(global.SYS_DB.Where("path = ?", "autoCodeEdit/:id").First(&system.SysBaseMenu{}).Error, gorm.ErrRecordNotFound) { // 判断是否存在数据
		return false
	}
	return true
}
